...
describe("UniswapV3 MULTI-SWAP", () =>
{
  async function deployFixture()
  {
    ...
    let ethAmount = 1n * 10n ** 18n; // 1 eth
  ...
  describe("swapMulti() [0.05 and 0.3]", () =>
  {
    it(" eth => usdc => gmx", async () =>
    {
      ...
      let poolAddress = await twap.getPoolAddress(WETH,
        usdc.address,
        gmx.address,
        500,
        3000);
      console.log(`Pool1 ${ poolAddress[1] } || Pool2 ${ poolAddress[1] }`)
  ...
  describe("swapMulti() [1 and 0.3]", () =>
  {
    ...
    {
      const { twap, owner, ethAmount, gmx, usdc } = await loadFixture(
        deployFixture
      );
      await twap.swapMulti(
        WETH,
  ...
  describe("swapMulti() [0.05]", () =>
  {
    ...
    {
      const { twap, owner, ethAmount, dai, usdc } = await loadFixture(
        deployFixture
      );
